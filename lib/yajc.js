    ...
    $.fn.enableSwipe = function() {
        return this.each(function() {
            ...
            var start = function(e) {
                active = true;
                startX = _.isNumber(e.pageX) ? e.pageX : e.originalEvent.targetTouches[0].pageX;
                e.preventDefault();
                console.log(startX)
            };

            var move = function(e) {
                var endX = _.isNumber(e.pageX) ? e.pageX : e.originalEvent.targetTouches[0].pageX,
                    diff = endX - startX;
                if (active && diff != 0) {
                    $list.trigger('swipe', {
            ...
            if (!supportTouch) $list.mouseup(end)
                                    .hover($.noop, end);
        });
    };

    $.fn.yajc.plugins.swipe = function(callback) {
        return this.enableSwipe().each(function() {
            var $root = $(this),
                $list = $root.find('.yajc > ul');
            
            $list.on('swipe', function(e, data) {
                var offset = data.start - data.end;
                $root.yajc('stop').yajc('slide', '+=' + offset, callback);
            });
        });
    };
