...
(function($) {
    ...
    var core = {
        ...
        refresh: function(callback) {
            return this.each(function() {
                ...
                // @TODO: animation here makes sense only when calling yajc('remove')
                // not when deleting from outside
     ...
     * Copyright 2012 Buu Nguyen
     *
     * Select an item when it is clicked or tap. Mark selected items with 'selected' class.
     * ========================================================== */
    $.fn.yajc.plugins.clickSelect = function(callback) {
        return this.each(function() {
            if (_.isTouchDevice()) return;
            
            var $root = $(this),
                evt = _.isTouchDevice() ? 'touchstart' : 'click';
            $root.find('li').live(evt,
                function(e) {
    ...
    $.fn.yajc.plugins.wings = function(style) {
        return this.each(function() {
            ...
            $bothWings.addClass(style = style || 'pad')
                .appendTo($root.find('.yajc'))
                .hide()
                .hover(
                    function() {
