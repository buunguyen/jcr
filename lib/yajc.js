...
(function($) {
    ...
    var core = {
            ...
            return this.each(function() {
                ...
                if (_.isString(to)) {
                    ...
                    if (to.indexOf('+=') === 0 || to.indexOf('-=') === 0)
                        to = (to.indexOf('-=') === 0 ? -1 : 1) * 
                             _.intValue(to.substring(2)) + 
                             from;
                    else to = _.intValue(to);
                }
        ...
        stop: function() {
            return this.each(function() {
                $(this).find('.items').stop(true, true);
            });                 
        },
            ...
            $list.mousemove(function(e) {
                var rootWidth = $root.width(),
                    ...
                    rootXOffset = $root.offset().left + 
                                  _.intValue($root.css('padding-left')) +
                                  _.intValue($root.css('margin-left')) +
                                  _.intValue($root.css('border-left')),
                    mouseXFromRootLeft = e.pageX - rootXOffset,
                    mouseXFromListLeft = (mouseXFromRootLeft * listWidth) / rootWidth;
                $root.yajc('stop').yajc('slide', -mouseXFromListLeft, callback);
            });
        });
    };
