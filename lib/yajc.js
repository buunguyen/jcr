...
(function($) {
    ...
    var core = {
        ...
        init: function(options) {
            return this.each(function() {
                var $root = $(this);
                $root.append('<div class="yajc"><ul></ul></div>')
                     .data('options', $.extend({}, $.fn.yajc.defaults, options));
        ...
        //  jumpToEnd (optional): true if want to jump to the end of the slide when stopping, default is true
        stop: function(jumpToEnd) {
            if (_.isUndefined(jumpToEnd)) jumpToEnd = true;

            return this.each(function() {
                $(this).find('.yajc > ul').stop(true, jumpToEnd);
            });                 
        },
        ...
        return this.each(function() {
            ...
            var slide = function(isForward) {
                if ($root.data('sliding')) {
                    if (options.snap) {
                        $root.yajc('show', (isForward ? 'next' : 'prev') + options.step);
                    } else {
    ...
    $.fn.yajc.plugins.swipe = function(callback) {
        return this.each(function() {
            var $root = $(this),
                $wrapper = $root.find('.yajc'),
            ...
            var end = function(e) {
                ...
                if (direction && elapsed < 300) {
                    ...
                    if (offset !== 0) {
                        ...
                        $root.yajc('stop').yajc('slide', '+=' + offset, cb);
                    }
                }
                active = false;
                e.preventDefault();
